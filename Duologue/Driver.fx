// Driver.fx — Final (ReShade 5.x DX10/11/12)
// Chain: Exposure -> Filmic -> Micro-Contrast -> Deband -> Sharpen
// Diversity Indexing + optional RTGI-aware tuning. Place AFTER RTGI.
...
uniform float DRV_DebandRange = 16.0; // px
uniform int DRV_DebandIters = 1; // 0..3 effective

// Sharpen
uniform float DRV_SharpStrength = 0.27;
uniform float DRV_SharpRadius = 2.0; // px
...
// RTGI-aware trim (set true when qUINT_RTGI is enabled and placed before Driver)
uniform bool DRV_RTGI_Mode = false;
uniform float DRV_RTGI_Attenuation = 0.85; // scales contrast/sharpen toward this
...
// Luma via coarse mips (stable & cheap)
...
float3 SharpenBilateral(float2 uv, float radius, float strength, float edge_guard)
{
    ...
    float edge = saturate(1.0 - grad * 2.0);
    float guard = lerp(edge, 1.0, edge_guard); // protect edges more when high gradient

    float3 usm = c + (c - blur) * (strength * guard);
    return saturate(usm);
}

// Deband + blue-noise (fixed 3-step, weight-gated; no dynamic break)
float3 Deband(float2 uv, float threshold, float range, int iterations, float strength)
{
    ...
    int iters = clamp(iterations, 0, 3);

    [unroll]
    for (int i = 0; i < 3; ++i)
    {
        float active = (i < iters) ? 1.0 : 0.0;
        ...
        float w1 = saturate(1.0 - dl1 / threshold) * active;
        float w2 = saturate(1.0 - dl2 / threshold) * active;
...
float3 DriverCore(float2 uv)
{
    ...
    float3 expc = col * exp2(EV);

    // Diversity + RTGI aware adapt
    ...
    // rtgi-aware attenuation
    float atten = DRV_RTGI_Mode ? DRV_RTGI_Attenuation : 1.0;
    adaptDetail *= atten;
    ...
    float3 final_col = ApplySaturation(mc, DRV_Saturation);

    return lerp(col, final_col, DRV_Strength);
}
...
float4 PS_DriverDeband(PSIn i) : SV_Target
{
    ...
    if (DRV_RTGI_Mode)
        adaptDeband *= 0.95; // tiny relax with RTGI
...
float4 PS_DriverSharpen(PSIn i) : SV_Target
{
    ...
    if (DRV_RTGI_Mode)
    {
        adaptDetail *= DRV_RTGI_Attenuation;
        adaptEdgeG += 0.05;
    }
